...
at_fluent = domain.add_fluent("at", p = step_type, i=step_type)
...
for action in orig_actions:
    ...
    apply_action = InstantaneousAction(f"apply_{action.name}", plan_step=step_type, cur_step=step_type, next_step=step_type, **variables_dict)
    apply_action.add_effect(mode_prog, False, condition=mode_prog)

    # fluent for plan step
    plan_fluent = domain.add_fluent(f"plan_{action.name}", p=step_type, i=step_type, **variables_dict)
    cur_step_param = apply_action.parameter("cur_step")
    next_step_param = apply_action.parameter("next_step")
    plan_step_param = apply_action.parameter("plan_step")
    block_params = [apply_action.parameter(f"b{i}") for i in range(1, num_params + 1)]
    apply_action.add_precondition(at_fluent(plan_step_param, cur_step_param))
    apply_action.add_precondition(plan_fluent(plan_step_param, cur_step_param, *block_params))
    apply_action.add_precondition(next_fluent(cur_step_param, next_step_param))

    apply_action.add_effect(at_fluent(plan_step_param, cur_step_param), False)
    apply_action.add_effect(at_fluent(plan_step_param, next_step_param), True)
...
for i, problem_str in enumerate(filtered_problems):
    ...
    # validate actions
    step_params = {f"i{i}" : step_type for i in range(1, max_step + 1)}
    validate_action = InstantaneousAction(f"validate_{t}", **step_params)
    prev_step_param = validate_action.parameter(f"i1")
    for j in range(2, max_step + 1):
        cur_step_param = validate_action.parameter(f"i{j}")
        validate_action.add_precondition(next_fluent(prev_step_param, cur_step_param))
        prev_step_param = cur_step_param
    ...
    step_params = [validate_action.parameter(f"i{j}") for j in range(1, max_step + 1)]
    validate_action.add_precondition(at_fluent(step_params[i], step_params[plan_lens[i]])) # this overshoots!
    validate_action.add_effect(at_fluent(step_params[i + 1], step_params[0]), True)
...
domain.set_initial_value(at_fluent(prev_step_object, prev_step_object), True)
...
for t, problem_str in enumerate(filtered_problems, start=1):
    plan_object = domain.object(f"i{t}")
    with open(f"./plans/plan-{problem_str}.txt", "r") as f:
        for i, line in enumerate(f, start=1):
            ...
            for object in parts[1:]:
                ...
                domain.set_initial_value(plan_fluent(plan_object, step_object, *[domain.object(obj_name) for obj_name in parts[1:]]), True)
